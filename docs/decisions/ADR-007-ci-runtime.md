...
008 documents how a repository runs Aker Build on `pull_request` and surfaces the scan + gate +
...
- The CLI bin is `packages/cli/src/bin.ts` — a **TypeScript** entry with **no build step and no
  published npm binary**. `npm i -g aker-build` does not exist.
...
- **Invoke the CLI in CI via corepack `pnpm` + `tsx`** (a TS runner), e.g.
  `pnpm dlx tsx packages/cli/src/bin.ts review-pr ...`, from a checkout of the Aker Build tooling. No
...
- **Grounded on what runs today**: documenting `npm i -g aker-build` or a packaged composite Action
...
- **Additive forward path**: a published `aker-build` binary and/or a packaged composite GitHub Action
  are later, additive steps behind the same documented chain — they don't change the contract.

## Alternatives considered

- **Assume a published `aker-build` binary** — none exists; would document a broken setup step.
...
- **Ship a live `.github/workflows/aker-build.yml`** — contradicts AC-008's four-place carve-out and
  auto-activates CI without the consumer's opt-in. Rejected.
- **Add a `aker-build review-pr --fail-on critical` flag** — would be the only testable code unit, but
