...
export function loadRuntimeConfig(env: Record<string, string | undefined> = process.env): RuntimeConfig {
  ...
  const tmpRootValue = env.AKER_BUILD_TMP_ROOT?.trim();
  if (env.AKER_BUILD_TMP_ROOT !== undefined && !tmpRootValue) throw new RuntimeConfigError("AKER_BUILD_TMP_ROOT");
  ...
  return {
    ...
    scanBudget: {
      ...
      maxTotalBytes: integer(
        env,
        "AKER_BUILD_SCAN_MAX_TOTAL_BYTES",
        250 * 1024 * 1024,
        1024 * 1024,
        2 * 1024 * 1024 * 1024,
      ),
    ...
    tmpRoot: resolve(tmpRootValue || join(tmpdir(), "aker-build-app")),
