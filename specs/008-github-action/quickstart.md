...
```yaml
# .github/workflows/aker-build.yml  (EXAMPLE — copy into your repo to enable)
name: Aker Build
on:
  pull_request:
...
jobs:
  review:
    ...
    steps:
      ...
      # Aker Build's CLI is TypeScript (no published binary yet) — run it via tsx (ADR-007).
      # Replace <aker-build-checkout> with how you vendor the tooling (submodule / checkout / future package).
      # `pnpm install` first so the workspace imports (@aker-build/*) resolve.
      - name: Run Aker Build (scan → review-pr <PR number>)
        env:
          GH_TOKEN: ${{ github.token }}        # `gh` auth in CI (no stored token)
        run: |
          cd <aker-build-checkout> && corepack pnpm install --frozen-lockfile && cd -
          corepack pnpm dlx tsx <aker-build-checkout>/packages/cli/src/bin.ts scan --out .aker-build
          ...
          corepack pnpm dlx tsx <aker-build-checkout>/packages/cli/src/bin.ts \
            review-pr ${{ github.event.pull_request.number }} --out .aker-build

      - name: Publish summary
        if: always()
        run: cat .aker-build/review.md >> "$GITHUB_STEP_SUMMARY"
      ...
      - name: Enforce critical gates
        if: ${{ vars.AKER_BUILD_FAIL_ON_CRITICAL == 'true' }}
        run: |
          crit=$(jq '[.findings[] | select(.severity == "critical")] | length' .aker-build/review.json)
          echo "critical findings: $crit"
          if [ "$crit" -gt 0 ]; then
            echo "::error::Aker Build: $crit critical gate finding(s) — failing the check."
            exit 1
          fi
```
...
- **Enable critical-blocking** by setting the repo variable `AKER_BUILD_FAIL_ON_CRITICAL=true`
...
- **`gh` auth + `pnpm install`**: PR mode needs `GH_TOKEN` (the job's `github.token`) and
  `pull-requests: read`; run `pnpm install` in the tooling checkout so `@aker-build/*` imports resolve.
...
| SC-007 Aker Build error → clear fail | a non-zero CLI step fails the job (`if: always()` summary still shows context) |
...
Copying this file into your repo activates CI on your PRs — that is **your** decision. Aker Build ships
the contract + this example + **ADR-007** (CI runtime/packaging), not a live workflow. A published
`aker-build` binary / packaged composite Action is a later, additive step.
