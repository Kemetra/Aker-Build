...
| `--out <dir>` | Output directory. Default: `./.aker-build/`. Outside scanned tracked source. |
...
| Output (file) | `project-map.json` validating against `@aker-build/project-map` (FR-002). |
...
| Aspect | Contract |
|--------|----------|
| `--out <dir>` / source | Reads the previously produced map from the designated path (default `./.aker-build/project-map.json`). |
...
- **Output conforms to 002** — validated with `@aker-build/project-map` before write (R5, SC-001).
